...
export default function Focus() {
  const { state, updateCourseTime } = useApp();
  ...
  const handleStop = () => {
    if (time > 0 && selectedCourseId) {
      ...
      updateCourseTime(selectedCourseId, hours);
      setSavedTime(hours);
      setTime(0);
  ...
  const studyPlan = calculateStudyPlan(state.settings, state.courses);
  ...
  return (
    <div className="min-h-screen pb-20 md:pb-0 md:pt-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 lg:py-12">
        ...
        {/* Timer Display */}
        <div className="card-apple p-8 md:p-12 lg:p-16 mb-6">
          <div className="text-center">
            <div className="mb-10 md:mb-12">
              <div className="text-8xl md:text-9xl lg:text-[12rem] font-light text-apple-text mb-6 font-mono tracking-tight">
                {formatTime(time)}
              </div>
              {selectedCourse && (
                <p className="text-xl md:text-2xl text-apple-text-secondary font-medium">
                  {selectedCourse.name}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 md:gap-4">
              {!isRunning ? (
                <button
                  ...
                  className="btn-apple flex items-center gap-2 px-8 py-4 text-base disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Play size={20} />
                  Démarrer
                </button>
                ...
                <button
                  onClick={handlePause}
                  className="btn-apple-secondary flex items-center gap-2 px-8 py-4 text-base"
                >
                  <Pause size={20} />
                  Pause
                </button>
              ...
              {time > 0 && (
                <>
                  <button
                    onClick={handleStop}
                    className="btn-apple flex items-center gap-2 bg-green-600 hover:bg-green-700 px-8 py-4 text-base"
                  >
                    <Square size={18} />
                    <span className="hidden sm:inline">Arrêter & Sauvegarder</span>
